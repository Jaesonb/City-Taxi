{{-- resources/views/passengers/trips.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Passenger Trips and Payments') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <h3 class="text-lg font-semibold mb-4">{{ __('Passenger Name') }}: {{ $passenger->name }}</h3>
                    <h4 class="text-md mb-4">{{ __('Email') }}: {{ $passenger->email }}</h4>

                    <h4 class="text-md font-semibold mb-4">{{ __('Trips and Payments') }}</h4>

                    @forelse($passenger->trips as $trip)
                    ...
                    @empty
                        <p>{{ __('No trips found for this passenger.') }}</p>
                    @endforelse
                </div>
            </div>
        </div>
